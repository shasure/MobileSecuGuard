package com.zsy.mobilesecuguard.chapter04.adapter;
...
import com.zsy.mobilesecuguard.utils.DensityUtil;
import com.zsy.mobilesecuguard.chapter04.utils.EngineUtils;
...
public class AppManagerAdapter extends BaseAdapter {
    ...
    /**
     * 先显示一个TextView表示下面都是用户APP,再显示用户App列表，再显示一个TextView，再加载系统App列表
     * @param position
     * @param convertView
     * @param parent
     * @return
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (position == 0) {
            ...
            tv.setText(String.format("用户程序：%d个", UserAppInfos.size()));
            return tv;
            // 系统应用
        } else if (position == (UserAppInfos.size() + 1)) {
            TextView tv = getTextView();
            tv.setText(String.format("系统程序：%d个", SystemAppInfos.size()));
            return tv;
        }
        // 获取到当前App的对象
        AppInfo appInfo;
